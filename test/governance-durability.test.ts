...
test('persists and restores the full governance snapshot locally', async () => {
  const folder = await mkdtemp(join(tmpdir(), 'enterprise-mcp-kit-')); try {
    ...
    assert.equal(restored.getPlan({ subjectId: 'reader', tenantId: 'northstar', roles: [] }, plan.id).actionType, 'netbox.device.update');
  } finally { await rm(folder, { recursive: true, force: true }); }
});
test('accepts only already-verified OIDC claims with matching issuer and audience', () => {
  const actor = resolveVerifiedOidcActor({ iss: 'https://issuer.example', aud: ['enterprise-mcp-kit'], sub: 'user-1', tenant_id: 'northstar', roles: ['planner'] }, { issuer: 'https://issuer.example', audience: 'enterprise-mcp-kit' });
  assert.deepEqual(actor, { subjectId: 'user-1', tenantId: 'northstar', roles: ['planner'] });
  assert.throws(() => resolveVerifiedOidcActor({ iss: 'https://other.example', aud: 'enterprise-mcp-kit', sub: 'user-1', tenant_id: 'northstar', roles: ['planner'] }, { issuer: 'https://issuer.example', audience: 'enterprise-mcp-kit' }), GovernanceAuthorizationError);
});
