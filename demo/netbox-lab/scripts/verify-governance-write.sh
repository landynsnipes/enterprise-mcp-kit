...
device_id="$(printf %s "$target" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const r=JSON.parse(s).results;if(r?.length!==1||r[0].tenant?.slug!=="northstar-financial"||r[0].custom_fields?.reconciliation_status!=="matched")process.exit(1);process.stdout.write(String(r[0].id))})')"
...
payload="$(DEVICE_ID="$device_id" LAST_UPDATED="$last_updated" EXPIRES="$expires" node -e 'process.stdout.write(JSON.stringify({actionType:"netbox.device.metadata.update",target:{kind:"netbox-device",id:process.env.DEVICE_ID},proposedChange:"Set recorded reconciliation status from matched to drifted.",confidence:1,evidence:[{source:`api/dcim/devices/${process.env.DEVICE_ID}/`,summary:"NetBox records matched at the captured version."}],ruleVersion:"bounded-metadata-v1",promptVersion:null,expiresAt:process.env.EXPIRES,operation:{field:"reconciliation_status",expectedValue:"matched",newValue:"drifted",expectedLastUpdated:process.env.LAST_UPDATED}}))')"
...
printf '%s\n%s' "$executed" "$replay" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const [a,b]=s.split("\n").map(JSON.parse);if(a.data?.state!=="executed"||a.data?.execution?.afterValue!=="drifted"||a.meta?.execution!=="enabled"||b.meta?.replayed!==true)process.exit(1)})'
...
printf %s "$rolled_back" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const a=JSON.parse(s);if(a.data?.state!=="rolled_back"||a.data?.rollback?.afterValue!=="matched")process.exit(1);console.log(JSON.stringify({bounded_write:"passed",state:a.data.state,restored:a.data.rollback.afterValue,live_electrical_or_routing_claim:false}))})'
