...
printf '%s\n%s' "$approval" "$replay" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const [a,r]=s.split("\n").map(JSON.parse);if(a.data?.state!=="approved"||a.data?.tenantId!=="northstar"||a.meta?.execution!=="disabled"||a.meta?.replayed!==false||r.data?.id!==a.data?.id||r.meta?.replayed!==true){console.error("Approval or replay proof failed.");process.exit(1)}console.log("governance-keycloak-prewrite-gate=passed")})'
