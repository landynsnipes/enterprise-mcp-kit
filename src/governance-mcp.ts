...
const versionValue = z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._+:-]{0,63}$/); const metadataField = z.enum(['reconciliation_status', 'observed_software_version', 'minimum_approved_version']);
...
const writeResult = z.object({ deviceId: z.number().int().positive(), deviceName: z.string(), tenantId: z.string(), field: metadataField, beforeValue: z.string().nullable(), afterValue: z.string().nullable(), beforeLastUpdated: z.string(), afterLastUpdated: z.string(), source: z.string() }).strict();
