...
const versionValue = z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._+:-]{0,63}$/); const metadataField = z.enum(['reconciliation_status', 'observed_software_version', 'minimum_approved_version', 'physical_address', 'shipping_address', 'description', 'facility', 'time_zone']);
const operation = z.object({ field: metadataField, expectedValue: z.union([reconciliationStatus, versionValue]).nullable(), newValue: z.union([reconciliationStatus, versionValue]).nullable(), expectedLastUpdated: z.string().datetime() }).strict();
...
const resultFields = { tenantId: z.string(), field: metadataField, beforeValue: z.string().nullable(), afterValue: z.string().nullable(), beforeLastUpdated: z.string(), afterLastUpdated: z.string(), source: z.string() }; const writeResult = z.union([z.object({ deviceId: z.number().int().positive(), deviceName: z.string(), ...resultFields }).strict(), z.object({ siteId: z.number().int().positive(), siteName: z.string(), ...resultFields }).strict()]);
const execution = writeResult.and(z.object({ executedBy: z.string(), executedAt: z.string() }).strict()); const rollback = writeResult.and(z.object({ rolledBackBy: z.string(), rolledBackAt: z.string() }).strict());
...
export function createGovernanceMcpServer(governance: InMemoryActionGovernance, actors: TrustedActorProvider): McpServer {
  ...
  server.registerTool('create_action_plan', { description: 'Create a tenant-scoped, evidence-backed, dry-run action plan. It never executes an external action.', inputSchema: planInput, outputSchema: planOutput }, (input) => respond(() => governance.createPlan(actors.getActor(), input as never)));
