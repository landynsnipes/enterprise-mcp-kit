...
export interface OidcJwksVerifierOptions { issuer: string; audience: string; jwksUrl: string; allowInsecureLoopback?: boolean; fetch?: Fetch; now?: () => Date; }
...
export class OidcJwksVerifier {
  ...
  async verify(token: string): Promise<VerifiedOidcClaims> {
    ...
    if (header.alg !== 'RS256' || !isText(header.kid)) throw new GovernanceAuthorizationError('Bearer token uses an unsupported signing algorithm.');
    ...
    const now = Math.floor(this.now().getTime() / 1000); if (!Number.isInteger(claims.exp) || (claims.exp as number) <= now || (claims.nbf !== undefined && (!Number.isInteger(claims.nbf) || (claims.nbf as number) > now)) || claims.iss !== this.options.issuer || !audiences(claims.aud).includes(this.options.audience)) throw new GovernanceAuthorizationError('Bearer token claims are invalid.');
    return { iss: claims.iss, aud: claims.aud, sub: claims.sub, tenant_id: claims.tenant_id, roles: claims.roles };
  }
}
