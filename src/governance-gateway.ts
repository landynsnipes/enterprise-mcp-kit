...
export interface GovernanceGatewayOptions { verifier: OidcJwksVerifier; store: GovernanceStore; issuer: string; audience: string; writer?: NetBoxDeviceMetadataWriter; siteWriter?: NetBoxSiteInformationWriter; provisioningAdapter?: ProvisioningDiscovery & SiteProvisioningAdapter; now?: () => Date; }
export class GovernanceGateway {
  ...
  async invoke(authorization: string | undefined, tool: GovernanceTool, input: unknown, idempotencyKey?: string): Promise<{ data: GovernanceGatewayData; meta: GatewayMeta }> {
    ...
    return { data: plan, meta: { requestId: randomUUID(), tenantId: actor.tenantId, execution: (this.options.writer && this.options.siteWriter)||this.options.provisioningAdapter ? 'enabled' : 'disabled', replayed } } as {data:GovernanceGatewayData;meta:GatewayMeta};
  }
}
