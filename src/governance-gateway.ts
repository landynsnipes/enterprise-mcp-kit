...
import { GovernanceAuthorizationError, GovernanceStateError, GovernanceValidationError, InMemoryActionGovernance, type ActionPlan, type GovernanceActor, type GovernanceAuditEvent, type GovernanceSnapshot } from './governance.js';
...
import { NetBoxInventoryIntentWriter, type InventoryIntent, type InventoryIntentWriteResult } from './netbox-inventory-intent-writer.js';
...
import { ProvisioningManifestError } from './site-provisioning-manifest.js';
import { NetBoxProvisioningError } from './netbox-site-provisioning-adapter.js';

export type GovernanceTool = 'create_action_plan' | 'plan_customer_site_provisioning' | 'get_action_plan' | 'list_audit_events' | 'approve_action_plan' | 'reject_action_plan' | 'execute_action_plan' | 'rollback_action_plan';
export type GovernanceAuditHistory = { planId: string; tenantId: string; events: GovernanceAuditEvent[]; boundary: string };
export type GovernanceGatewayData = ActionPlan | GovernanceAuditHistory;
type GatewayMeta={requestId:string;tenantId:string;execution:'disabled'|'enabled';replayed:boolean};
export interface GovernanceGatewayOptions { verifier: OidcJwksVerifier; store: GovernanceStore; issuer: string; audience: string; writer?: NetBoxDeviceMetadataWriter; siteWriter?: NetBoxSiteInformationWriter; inventoryWriter?: NetBoxInventoryIntentWriter; provisioningAdapter?: ProvisioningDiscovery & SiteProvisioningAdapter; now?: () => Date; }
export class GovernanceGateway {
  constructor(private readonly options: GovernanceGatewayOptions) {}
  async authenticate(authorization: string | undefined): Promise<GovernanceActor> { const token=authorization?.match(/^Bearer ([^\s]+)$/i)?.[1];if(!token)throw new GovernanceAuthorizationError('Bearer authentication is required.');return resolveVerifiedOidcActor(await this.options.verifier.verify(token),{issuer:this.options.issuer,audience:this.options.audience}); }
  async invoke(authorization:string|undefined,tool:'list_audit_events',input:unknown,idempotencyKey?:string):Promise<{data:GovernanceAuditHistory;meta:GatewayMeta}>;
  async invoke(authorization:string|undefined,tool:Exclude<GovernanceTool,'list_audit_events'>,input:unknown,idempotencyKey?:string):Promise<{data:ActionPlan;meta:GatewayMeta}>;
  async invoke(authorization:string|undefined,tool:GovernanceTool,input:unknown,idempotencyKey?:string):Promise<{data:GovernanceGatewayData;meta:GatewayMeta}>;
  async invoke(authorization: string | undefined, tool: GovernanceTool, input: unknown, idempotencyKey?: string): Promise<{ data: GovernanceGatewayData; meta: GatewayMeta }> {
    const actor = await this.authenticate(authorization); const value = input as Record<string, unknown>; const mutating = !['get_action_plan','list_audit_events'].includes(tool);
    ...
    const plan = await this.options.store.transact(async ({ receipts, ...stored }) => {
      ...
      const governance = new InMemoryActionGovernance(this.options.now); governance.restore(snapshot); let result: GovernanceGatewayData;
      ...
      else if (tool === 'list_audit_events') { const planId=requireText(value.planId);result={planId,tenantId:actor.tenantId,events:governance.listAuditEvents(actor,planId),boundary:'Tenant-scoped governance lifecycle events only; credentials and raw request payloads are excluded.'}; }
      ...
      const next: GovernanceSnapshot = { ...governance.snapshot(), receipts: [...receipts] }; if (mutating) {if(!('id'in result))throw new GovernanceStateError('Governance mutation did not return an action plan.');next.receipts.push({ key: idempotencyKey!, tenantId: actor.tenantId, actorId: actor.subjectId, operation: tool, requestDigest: digest, planId: result.id, recordedAt: (this.options.now ?? (() => new Date()))().toISOString() });} return { snapshot: next, result };
    });
    return { data: plan, meta: { requestId: randomUUID(), tenantId: actor.tenantId, execution: (this.options.writer && this.options.siteWriter)||this.options.inventoryWriter||this.options.provisioningAdapter ? 'enabled' : 'disabled', replayed } } as {data:GovernanceGatewayData;meta:GatewayMeta};
  }
}
...
export function safeGatewayError(error: unknown): { error_code: string; message: string } { if (error instanceof GovernanceAuthorizationError) return { error_code: 'AUTHORIZATION_DENIED', message: error.message }; if (error instanceof GovernanceValidationError || error instanceof ProvisioningManifestError || error instanceof NetBoxProvisioningError) return { error_code: 'VALIDATION_ERROR', message: error.message }; if (error instanceof GovernanceStateError) return { error_code: 'GOVERNANCE_STATE_ERROR', message: error.message }; return { error_code: 'GOVERNANCE_REQUEST_FAILED', message: 'Governance request failed.' }; }
export function createGovernanceHttpHandler(gateway: GovernanceGateway) {
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    const requestId = typeof request.headers['x-request-id'] === 'string' ? request.headers['x-request-id'] : randomUUID(); const match = request.url?.match(/^\/api\/v1\/governance\/tools\/(create_action_plan|plan_customer_site_provisioning|get_action_plan|list_audit_events|approve_action_plan|reject_action_plan|execute_action_plan|rollback_action_plan)$/);
...
function safeExecutionReason(error: unknown): string { const result=(error as {result?:{errors?:unknown}})?.result; if(Array.isArray(result?.errors)&&typeof result.errors[0]==='string'&&result.errors[0].length<=240)return result.errors[0]; return error instanceof Error && error.message.length <= 240 ? error.message : 'Bounded NetBox write failed.'; }
function isInventoryIntent(value: unknown): value is InventoryIntent { return !!value && typeof value === 'object' && 'action' in value && 'targetKind' in value && 'targetId' in value; }
function isInventoryResult(value: unknown): value is InventoryIntentWriteResult { return !!value && typeof value === 'object' && 'recordType' in value && 'recordId' in value && 'before' in value && 'after' in value; }
