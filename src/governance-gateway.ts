...
export function createGovernanceHttpHandler(gateway: GovernanceGateway) {
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    const requestId = typeof request.headers['x-request-id'] === 'string' ? request.headers['x-request-id'] : randomUUID(); const match = request.url?.match(/^\/api\/v1\/governance\/tools\/(create_action_plan|get_action_plan|approve_action_plan|reject_action_plan|execute_action_plan|rollback_action_plan)$/);
